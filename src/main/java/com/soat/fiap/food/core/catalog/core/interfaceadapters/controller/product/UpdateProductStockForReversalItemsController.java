...
public class UpdateProductStockForReversalItemsController {
	 ...
	 * solicitada em um pedido, em cenários de estorno.
	 *
	 ...
	 */
	public static void updateProductStockForReversalItems(StockDebitEventDto stockDebitEventDto,
		...
		var productStockUpdateInput = ProductStockUpdateMapper.toInputFromStockDebit(stockDebitEventDto.getItems());
		...
		for (ProductStockUpdateInput.ProductStockItemInput productStockItemInput : productStockUpdateInput.items()) {
			...
			log.info(
					"Atualização de quantidade em estoque realizada com sucesso! Motivo: Estorno de Pedido, ProductId {}",
					productStockItemInput.productId());
		}

		PublishStockReversalEventUseCase.publishStockReversalEvent(stockDebitEventDto.orderId, eventPublisherGateway);
	}
}
