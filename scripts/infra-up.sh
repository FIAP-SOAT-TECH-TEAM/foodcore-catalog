...
RUNNING_DB=$(docker ps --filter "name=foodcore-catalog-ms-db" --format "{{.Names}}")
...
if [ -n "$RUNNING_DB" ]; then
  ...
  docker-compose stop catalog-ms-db
fi
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
    if docker-compose exec catalog-ms-db pg_isready -q; then
...
echo "Acesso: localhost:5434 (usuário: postgres)"